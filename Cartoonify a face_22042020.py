...
def cartoonize_image(img, ds_factor=4, sketch_mode=False):
    ...
    #Apply median filter to the grayscale
    img_gray = cv2.medianBlur(img_gray, 5)

    #Detect edges in the image & threshold it
    ...
    #Resize the image to a smaller for faster computation
    img_small = cv2.resize(img, None, fx=10.0/ds_factor, fy=10.0/ds_factor, interpolation = cv2.INTER_AREA)
    num_repetitions = 10
    sigma_color = 5
    sigma_space = 7
    size = 5

    #Apply bilateral filter on the image multiple times
